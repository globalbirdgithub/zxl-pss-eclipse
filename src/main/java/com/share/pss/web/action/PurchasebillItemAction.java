...
public class PurchasebillItemAction extends CRUDAction<PurchasebillItem>{
	...
	@Override
	protected void list() {
		this.pageList = purchasebillItemService.findByQuery(baseQuery);
	}
	//新建/修改
	@Override
	...
	protected void savee() {
		...
		purchasebillItemService.saveOrUpdate(purchasebillItem);
	}
	//ajax删除
	@Override
	...
	@Override
	protected PurchasebillItem getModell() {
	...
	protected void prepareInputt(){
		if(id!=null){
			purchasebillItem = purchasebillItemService.get(id);//修改需要回显否则不需要(这时会压栈)
		}
	}
	@Override
