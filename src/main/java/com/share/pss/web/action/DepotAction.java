 ...
 * @version V1.0 表现层Action 访问Action的时候默认Action在栈顶，ModelDriven对应的拦截器检测到Depot有值的时候将其压栈到栈顶
...
public class DepotAction extends CRUDAction<Depot>{
	...
	protected void prepareInputt(){
		if(id!=null){
			depot = depotService.get(id);//修改需要回显否则不需要(这时会压栈)
		}
	}
	@Override
