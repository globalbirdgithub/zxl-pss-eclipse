 ...
 * @version V1.0 表现层Action 访问Action的时候默认Action在栈顶，ModelDriven对应的拦截器检测到ProductType有值的时候将其压栈到栈顶
...
public class ProductTypeAction extends CRUDAction<ProductType>{
	...
	@Override
	protected ProductType getModell() {
		return productType;
	}
	...
	protected void prepareInputt(){
		if(id!=null){
			productType = productTypeService.get(id);//修改需要回显否则不需要(这时会压栈)
		}
	}
	@Override
