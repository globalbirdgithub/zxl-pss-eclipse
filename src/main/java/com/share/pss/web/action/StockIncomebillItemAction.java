...
import com.share.pss.service.IStockIncomebillItemService;
/**
 * @author MrZhang
 * @date 2017年11月1日 下午11:32:44
 * @version V1.0 表现层Action 访问Action的时候默认Action在栈顶，ModelDriven对应的拦截器检测到StockIncomebillItem有值的时候将其压栈到栈顶
...
public class StockIncomebillItemAction extends CRUDAction<StockIncomebillItem>{
	...
	protected void savee() {
		...
		stockIncomebillItemService.saveOrUpdate(stockIncomebillItem);
	}
	//ajax删除
	@Override
	...
	@Override
	protected StockIncomebillItem getModell() {
	...
	protected void prepareInputt(){
		if(id!=null){
			stockIncomebillItem = stockIncomebillItemService.get(id);//修改需要回显否则不需要(这时会压栈)
		}
	}
	@Override
